...
namespace utils {
  ...
  {
    // TODO: To fully benefit from the asynchronicity we should allocate the host memory
    // using `cudaMallocHost`. See here (slide 11 and 21 beyond):
    // https://developer.download.nvidia.com/CUDA/training/StreamsAndConcurrencyWebinar.pdf
    // Typically we would like to probably persist the tiles generated by the fractal in
    // some way. This makes sense because unless the window is resized the tiles will always
    // be the same. It would allow to use the `CudaJob` interface to maybe allocate pinned
    // memory for the internal data of the tile and thus benefit from concurrent memcpy.
